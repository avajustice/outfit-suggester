(function(){
    ...
    const profileNone = document.getElementById("profile-none");
    ...
    // Checks if the specified URL is a valid image URL
    ...
    // Update the contents of the profile page and user icon
    // based on the user's authentication status.
    // This does NOT display the profile page.
    async function updateProfilePage() {
        const profile = await getProfile();
        console.log(profile);
        if(profile.isAuthenticated) {
            // Show the user profile and hide the login option
            profileUser.style.display = "block";
            profileNone.style.display = "none";

            // Show the user name
            if (profile.user) {
                profileUserName.innerText = profile.user.name;
            }

            // If the returned image is valid, set it as the
            // profile photo and the user icon.
            if(await isValidImageUrl(profile.user.picture)) {
                // Show the user profile picture
                profileImage.src = profile.user.picture;
                userIcon.src = profile.user.picture;
            } else {
    ...
    async function displayProfilePage() {
        // Display only the profile elements
        hideAll();
    ...
    async function retrieveItemsFromDatabase() {
        ...
        if (!rawResponse.ok) {
            console.error('Failed to retrieve items from the database');
    ...
    getProfile = async function() {
        const rawResponse = await fetch(webServiceURL + 'api/profile', {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
    ...
    async function getDisplayWeekHistory() {
        ...
        for (i = 0; i < 7; i++) {
            ...
            if (dateData != "Failed" && dateData.itemIDs) {
    ...
    profileButton.onclick = displayProfilePage;
