(function(){
    ...
    async function retrieveDisplayItemsFromDatabase() {
        ...
        for (const item of responseItemArray)
        {
            createItemObject(item.name, item.clothingType, item.color, item.shortLong,
                item.patterned, item.available, item.washType, item.lastWorn, item.id, 
                item.imgId);
        }
    ...
    function Item(name, clothingType, color, shortLong, patterned, available, washType, lastWorn, id, imgId) {
        this.name = name;
        ...
        this.addItemToDatabase = async function() {
            ...
            const rawResponse = await fetch('https://outfit-suggester-service.avajustice.repl.co/api/items', {
                ...
                body: JSON.stringify({"name" : this.name, "clothingType" : this.clothingType, 
                    ...
                    "patterned" : this.patterned, "available" : this.available,
    ...
    addNewItem = async function(name, type, color, shortLong, wash, lastWorn) {
        ...
        const item = createItemObject(name, type, color, shortLong, patterned, available, wash, lastWorn, 0, imgId);

        const id = await item.addItemToDatabase();
        item.id = id;
        item.updateItemInDatabase(name, type, color, shortLong, patterned, available, wash, lastWorn, id, imgId);
    ...
    function createItemObject(name, type, color, shortLong, patterned, available, wash, lastWorn, id, imgId) {
        ...
        const item = new Item(name, type, color, shortLong, patterned, available, wash, lastWorn, id, imgId);
    ...
    createItemButton.addEventListener('click', function(){
        addNewItem(nameSelect.value, typeSelect.value, colorSelect.value, 
            ...
            washSelect.value, lastWornSelect.value);
    });
